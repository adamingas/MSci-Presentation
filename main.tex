...
\begin{itemize}
	...
	\pause
	\item Barcoding and taxonomic reference libraries
	\pause
	\item Next Generation Sequencing 
	\pause
	\item Costs go down by orders of magnitude  
\end{itemize}
\end{frame}
\note[itemize]{
...
\item Costs went down by orders of magnitude  }
\begin{frame}
	\frametitle{\LARGE Genomics; A new hope?}
	\begin{columns}
...
\end{frame}
\note{
Sequncing => OTUs; genetic proxies for species. Sequences clustered with 97\% similarity and are given a signle otu name and taxonomy. Number of sequences => read counts}
	...
	\section{Data}
		\begin{frame}
			...
			\end{itemize}}   
			\only<2>{\includegraphics[width=\textwidth]{map1}}
			\only<3>{\includegraphics[width=\textwidth]{selectedrivers}}
		\end{frame}
\note[itemize]{
\item
\item
...
\item PCoA is generalisation which does eigen analysis on a differetnet distance metric than euclidean. Used bray curtis commonly used in ecological studies
\item nmds preserves order relations in dissimilarities between samples in the projection of data. EXAMPLE
